...
@Repository
public interface ReimbursementRepository extends JpaRepository<ReimbursementEntity, Integer> {
...
//    Optional<UserEntity> findUserEntityByUsername(String name);
    @Query(value = "SELECT s FROM reimbursement_request s WHERE s.employeename = ?1", nativeQuery = true)
    Optional<ReimbursementEntity> findByUsername(String name);     // ??????????????
    // ??????????????
    // ??????????????
    // ??????????????
    // ??????????????
